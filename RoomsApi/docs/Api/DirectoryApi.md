# WebMI\RocketChatApiClient\RoomsApi\DirectoryApi
...
```php
getApiV1Directory($x_auth_token, $x_user_id, $query, $text, $type, $workspace, $offset, $count, $sort, $fields): \WebMI\RocketChatApiClient\RoomsApi\Model\GetApiV1Directory200Response
```
...
$text = rocket; // string | The search term to be filtered
$type = users; // string | It can be `channels` or `users`. When `type` is users you can send an additional workspace field, that can be local (default) or all. Workspace=all will work only if <a href=\"https://docs.rocket.chat/use-rocket.chat/rocket.chat-federation\" target=\"_blank\">federation</a> is enabled.
$workspace = local; // string | It can be `local` or `all`( if federation is enabled). It is `local` by default.
